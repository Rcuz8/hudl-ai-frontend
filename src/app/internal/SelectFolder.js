...
function JSONHolder(json, callback) {
  return (
    <div class="jsonwrapper">
      <JSONComponent data={json} depth={0} callback={callback} />
    </div>
  );
}
...
class JSONComponent extends React.Component {
  ...
  handlecallback = (arr, name) => {
    if (this.props.index !== undefined) arr.push(this.props.index);
          ...
          return (
            <div>
              <div
                class="json"
                style={{
                  marginLeft: 15 * (depth + 1),
                  color: next_exist ? "black" : "white",
                  backgroundColor: next_exist ? calcColor(depth) : "black",
                }}
                onClick={() => this.toggle(child, i)}
              >
                {child}
              </div>
    ...
    ws.onopen = function () {
      ...
      ws.send(
        JSON.stringify({
          ...
          dir_list: Selection_arr.toString(),
        })
      );
