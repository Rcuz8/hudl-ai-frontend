import React, {
  ...
  export default function SelectTestData(props) {
    ...
    const TRAINING_DATA_PREPARED = props.str_trainingdata
    ...
    const Selected_Film_Clip = (arr, name) => {
      ...
      if (TRAINING_ARRs.indexOf(item) !== -1) alert('Already selected this for training!');
    ...
    const submit = () => {
      ...
      ws.onmessage = function(e) {
        ...
        if (!combined_data) {
          ...
          let clean_parse_test = cleaned_withParams(ALL_FILM_DATA_COMBINED.split(TKN_2), USED_INDICES)
